...
func GetTest(technique, name, version string) (*Test, error) {
	...
	return nil, fmt.Errorf("test '%s' not found for technique '%s'", name, technique)
}

// GetTestByGuid fetches a specific test by technique and GUID
func GetTestByGuid(technique, guid, version string) (*Test, error) {
	tech, err := GetTechnique(technique, version)
	if err != nil {
		return nil, err
	}
	...
	for _, test := range tech.AtomicTests {
		if test.AutoGeneratedGUID == guid {
			return &test, nil
		}
	}

	return nil, fmt.Errorf("test with GUID '%s' not found in technique '%s'", guid, technique)
}
